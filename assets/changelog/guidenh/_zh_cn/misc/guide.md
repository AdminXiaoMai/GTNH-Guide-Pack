...
GuideNH 是 2.9 中全新的一个模组，目标是把过去埋在任务书里，或者散落在 wiki 等站外页面上的信息，更直接地搬进游戏内。
...
把鼠标移到左侧边缘，会弹出“导航”栏。这里会显示各份指南定义的“顶级”分类。大多数指南通常会按模组分区（例如 AE2 指南放在 AppliedEnergistics2 分类下），当然也有例外，比如你现在正在读的 changelog。
...
这个模组支持的功能其实多到没法在这里认真列完，所以这里只能半随机地拼一点展示内容，让你大概感受一下它能渲染出哪些花样：
...
<Latex tooltip="看吧妈妈！有 tooltip！" color="FF55FF" formula="\text{EU/t} (\leq \dot{m}^*) = \dot{m} \times \Biggl( 1 - \frac{|\dot{m} - \dot{m}^*|}{\dot{m}^*} \Biggr) \times 1.0 \times \eta">
...
GuideNH 使用 <Color id="GREEN">Markdown</Color> 页面驱动，搭配 YAML frontmatter 和 MDX 风格的运行时标签。页面通过放入某个语言文件夹（例如 `_en_us`）来声明所属语言。
