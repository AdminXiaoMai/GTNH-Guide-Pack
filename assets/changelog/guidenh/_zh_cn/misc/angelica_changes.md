...
这个模组本身并不新，但 2.9 里还是有一些变化。这里还没算上大量常规渲染优化和 bug 修复。
...
后台已经有不少工作在推进，目标是让 <Color id="GREEN">高级颜色追踪</Color> 成为 GTNH 的默认表现。已经在用 Euphoria Patches 一类方案的人对这个概念应该不陌生；总之，$$\text{soon}^{TM}$$，它将会对任意光影提供完整支持（甚至可能不装光影也行！）
