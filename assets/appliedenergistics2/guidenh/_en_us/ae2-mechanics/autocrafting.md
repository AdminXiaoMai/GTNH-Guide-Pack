...
3.  The <ItemLink id="appliedenergistics2:tile.BlockInterface" /> with the relevant [pattern](../items-blocks/patterns.md) pushes the ingredients specified in the pattern to any adjacent inventory.
...
*   (Required) [Crafting storages](../items-blocks/crafting_cpu.md), available in all the standard cell sizes (1k, 4k, 16k, 64k, 256k). They store the ingredients and
...
their [patterns](../items-blocks/patterns.md) to adjacent inventories, and items can be inserted into them in order to insert them into the network. Often
...
Of note, since they push the ingredients directly from the [crafting storage](../items-blocks/crafting_cpu.md#crafting-storage) in a crafting CPU, they
...
the interface will skip the interface entirely and push directly to that subnet's [storage](../ae2-mechanics/import-export-storage.md),
...
Priorities can be set by clicking the wrench in the top-right of the GUI. In the case of several [patterns](../items-blocks/patterns.md)
