...
into [network storage](../ae2-mechanics/import-export-storage.md). It can process up to nine item groups per tick, and can
...
* Attaching an <ItemLink id="appliedenergistics2:item.ItemMultiPart:220" /> to an unconfigured interface makes the storage bus
  access all [storage](../ae2-mechanics/import-export-storage.md) on the interface's network, as though it were attached to
...
* An unconfigured interface on a [subnetwork](../ae2-mechanics/subnetworks.md) lets a connected ME interface push directly
...
Interfaces have a normal block form and a flat [cable subpart](../ae2-mechanics/cables-subparts.md):
...
* <ItemLink id="appliedenergistics2:item.ItemMultiMaterial:53" /> enables autocrafting requests, extracting stored items first
  and triggering [autocrafting](../ae2-mechanics/autocrafting.md) when storage is insufficient.
