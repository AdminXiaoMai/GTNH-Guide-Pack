...
The Annihilation Plane is a [cable subpart](../ae2-mechanics/cables-subparts.md).

**REMEMBER TO ENABLE FAKE PLAYERS IN YOUR CHUNK CLAIM**

## Filtering

The Annihilation Plane will only break a block or pick up an item if the drops or items can be stored in its network. This means to filter one, *you must restrict what can be stored on its network*, most likely by putting it on a [subnetwork](../ae2-mechanics/subnetworks.md). An <ItemLink id="appliedenergistics2:item.ItemMultiPart:220" /> or [partitioning](cell_workbench.md) the [cells](storage_cells.md) can achieve this.
