...
* **Fuzzy Comparison**: Available when a <ItemLink id="appliedenergistics2:item.ItemMultiMaterial:29" showIcon="left" linksTo="../items-blocks/upgrade_cards.md#Fuzzy Card"/> is installed. Adjusts how partitions perform fuzzy matching.
* **Ore Dictionary Filter**: Available when a <ItemLink id="appliedenergistics2:item.ItemMultiMaterial:55" showIcon="left" linksTo="../items-blocks/upgrade_cards.md#Ore Dictionary Filter Card"/> is installed. Allows ore dictionary names to be entered for filtering. When enabled, the normal partition slots are hidden, and it takes priority over fuzzy mode.
