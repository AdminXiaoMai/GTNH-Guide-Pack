...
* **矿典过滤**：安装<ItemLink id="appliedenergistics2:item.ItemMultiMaterial:55" showIcon="left" linksTo="../items-blocks/upgrade_cards.md#矿典过滤卡"/>后可用，用于调后可用，可输入矿典名进行过滤。启用后会隐藏普通分区槽，并优先于模糊模式。
* **限制设置**：点击后可进入GUI设置：
