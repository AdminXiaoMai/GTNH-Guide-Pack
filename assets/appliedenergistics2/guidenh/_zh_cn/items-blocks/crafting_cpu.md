...
（可选）并行处理单元通过提升CPU运转速度让系统从<ItemLink id="appliedenergistics2:tile.BlockInterface" />更为频繁地发送材料批次，以使系统跟上处理速度较快的机器。例如，被<ItemLink id="appliedenergistics2:tile.BlockMolecularAssembler" />包围的样板供应器送出材料的速度快于单个装配室的加工速度时，其会将材料批次在各装配室间分配。
